...
";
      ...
      public override bool Open(string path, int line, int column, string solution)
      {
         ...
         var solutionDirectory = IOPath.GetDirectoryName(solution);
         if (string.IsNullOrEmpty(solutionDirectory))
            return false; // Handle the error as needed

         // Get the parent directory one level above the solution directory
         var parentDirectoryInfo = Directory.GetParent(solutionDirectory);
         if (parentDirectoryInfo == null)
            return false; // Handle the error as needed

         var parentDirectory = parentDirectoryInfo.FullName;

         // Find the first *.code-workspace file in the parent directory
         var workspaceFilePath = Directory
            .EnumerateFiles(parentDirectory, "*.code-workspace")
            .FirstOrDefault();
         ...
         // Start the process to open the workspace file
         ProcessRunner.Start(string.IsNullOrEmpty(path) ?
            ProcessStartInfoFor(application, $"\"{workspaceFilePath}\"") :
            ProcessStartInfoFor(application, $"\"{workspaceFilePath}\" -g \"{path}\":{line}:{column}"));
